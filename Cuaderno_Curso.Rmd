...
# En despliegues con rsconnect/Posit Connect, Python se crea desde
# requirements.txt y suele exponerse mediante RETICULATE_PYTHON.
connect_python <- Sys.getenv("RETICULATE_PYTHON", unset = "")

if (nzchar(connect_python)) {
  use_python(connect_python, required = TRUE)
} else {
  conda_available <- tryCatch({
    conda <- conda_binary("auto")
    nzchar(conda) && file.exists(conda)
  }, error = function(e) FALSE)

  conda_envs <- if (conda_available) {
    tryCatch(conda_list()$name, error = function(e) character())
  } else {
    character()
  }

  if ("py311_arm" %in% conda_envs) {
    use_condaenv("py311_arm", required = TRUE)
  } else {
    message("No se encontro conda/py311_arm; reticulate usara el Python disponible.")
  }
}
...
Ese bloque de código en R configura la integración con Python mediante la librería reticulate. En despliegues con rsconnect o Posit Connect, usa el intérprete definido en la variable de entorno `RETICULATE_PYTHON`, creado a partir de `requirements.txt`. En ejecución local, intenta usar el entorno Conda `py311_arm` si está disponible; si no lo encuentra, deja que reticulate use el Python disponible. Finalmente ejecuta `py_config()` para mostrar la configuración activa, confirmando qué versión de Python y qué entorno se están utilizando.
...
import plotly.io as pio                   # Configuración de renderizadores Plotly
import scipy as sc                        # Acceso al namespace general de SciPy
...
```{python message=FALSE, warning=FALSE}
# Boxplots
sns.boxplot(x="Species", y="petal length (cm)", data=iris, linewidth=2.5)
plt.show()
```
...
```{python fig.height=15, fig.width=15, message=FALSE, warning=FALSE}
sns.histplot(X[Y == 0, 2], bins=10, kde=True, stat="density", element="step", label="setosa")
sns.histplot(X[Y == 1, 2], bins=10, kde=True, stat="density", element="step", label="versicolor")
sns.histplot(X[Y == 2, 2], bins=10, kde=True, stat="density", element="step", label="virginica")
plt.legend()
plt.show()
```
...
```{python fig.height=15, fig.width=15, message=FALSE, warning=FALSE}
sns.histplot(X[:, 1], bins=10, kde=True, stat="density")
plt.show()
```
